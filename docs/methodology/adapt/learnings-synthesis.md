...
```bash
jq --arg v "{VERTICAL}" '.sessions[] | select(.vertical == $v)' ~/Github/<your-project>-worktrees/.session-registry.json
```
...
gh pr list --repo <your-owner>/<your-repo> --state merged --search "{VERTICAL}" --limit 20 --json number,title,additions,deletions,reviews
```
...
```bash
gh pr list --repo <your-owner>/<your-repo> --state merged --search "{VERTICAL}" --json number --jq '.[].number' | head -5
```

For each PR, skim reviews: `gh api repos/<your-owner>/<your-repo>/pulls/{N}/reviews`

**Open issues** related to the vertical:

```bash
gh issue list --repo <your-owner>/<your-repo> --label "product/{VERTICAL}" --state open
# or for domain-scoped work:
gh issue list --repo <your-owner>/<your-repo> --label "domain/{VERTICAL}" --state open
```
