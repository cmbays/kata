...
export interface WorkflowRunOptions {
  ...
  pin?: string[];
  dryRun?: boolean;
  /** ID of the agent driving this run — stored in artifact metadata. */
  agentId?: string;
...
/**
 * WorkflowRunner — the primary service for executing a Stage orchestration.
 *
 * Bridges the CLI (`kata execute`, alias `kata kiai`) with the Stage Orchestrator.
...
export class WorkflowRunner {
  ...
  async runPipeline(
    ...
    const result = await metaOrchestrator.runPipeline(categories, options.bet, {
      yolo: options.yolo,
      flavorHints: options.flavorHints,
      agentId,
      katakaId: agentId,
    });
    const completedAt = new Date().toISOString();
...
/**
 * List recent stage artifacts from .kata/artifacts/.
 * Standalone function usable without full WorkflowRunner initialization.
 */
