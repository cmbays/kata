...
import { KataAgentRegistry } from './kata-agent-registry.js';
...
describe('KataAgentRegistry', () => {
  let basePath: string;
  let registry: KataAgentRegistry;

  beforeEach(() => {
    basePath = mkdtempSync(join(tmpdir(), 'kataka-registry-test-'));
    registry = new KataAgentRegistry(basePath);
  });
  ...
  describe('get', () => {
    ...
    it('loads from disk when not in cache', () => {
      ...
      const fresh = new KataAgentRegistry(basePath);
